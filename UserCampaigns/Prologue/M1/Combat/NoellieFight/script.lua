...
noellie_fight =
{
    path = GetMapDataPath().."Combat/NoellieFight/",
    ...
    channel_cast_turn = 5 - initialDifficulty,
    ...
    chaos_spell_round_progress = initialDifficulty == DIFFICULTY_HEROIC and 0.6 or 0.4,
    ...
    noellie_atb_return_factor = 0.3 + 0.03 * initialDifficulty
}

-- призыв ведьм
function NoellieFight_SummonWitches()
    DisableCombat()
    pcall(SummonCreature, DEFENDER, CREATURE_COVEN_LEADER, 8 + 2 * initialDifficulty, 9, 13, 1, "coven_leader")
    ...
    for witch, info in noellie_fight.witches_info do
        if not exist(witch) then
            pcall(SummonCreature, DEFENDER, info.type, 8 + 2 * initialDifficulty, info.x, info.y, 1, witch)
        end
    end
    EnableCombat()
end
...
function NoellieFight_NoellieMove(hero)
    ...
    -- 2.
    local spell = Random.FromTable(noellie_fight.chaos_spells_groups[current_round])
    local spell_type = Spell.Params.Type(spell)
    ...
    if spell_type == AIMED then
        ...
        for i, creature in GetCreatures(side) do
            current_powers[creature] = Creature.Params.Power(creature) * GetCreatureNumber(creature)
        end
        ...
        -- 3b.
        creature_to_cast = Random.FromTable(stacks_to_select)
