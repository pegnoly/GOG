--------------------------------------------------------------------------------
-- все связанное с Моритоном: нападения нежити, квесты Греты и прочее
...
moriton =
{
  ...
  -- табличка для удобства вызова PlayVisualEffects/CreateMonster в сценке атаки нежити
  undead_units =
  {
  ...
  reinfs = 0, -- день прибытия подкреплений
  first_reinf = 0, -- флаг первого подкрепления
  remove_def = 0, -- флаг снятия защиты с города
  
  deadStacks = 0, -- счетчик убитых стеков нежити
  
  first_enter = 0, -- флаг первого входа в подземку
  
  wsp_learned = 0, -- флаг изучения умелки у ведьмы
  
  -- для тестов героев - некров
  test_day = 0,
  test_hero = 'Orris'
}

--------------------------------------------------------------------------------
-- проигрыш в случае потери города
...
-- гарнизон, блокирующий проход во 2 локу
...
OutGarSetup()
--------------------------------------------------------------------------------
-- атака нежити при уходе Карлама из города
...
function MoritonAttackEvent()
  ...
  for i, unit in moriton.undead_units do
    startThread(
    function()
      local x, y = 30, 75 -- координаты всех эффектов/призывов формируются относительно конюшни рядом
      local unit = %unit
      local i = %i
...
-- настройка героев - некров. Вынесена отдельно, в основном, чтобы облегчить тесты
...
-- прибытие подкреплений в город
function CheckMoritonReinforces(day)
  if day == moriton.reinfs then
    moriton.reinfs = moriton.reinfs + 7
    if moriton.first_reinf == 0 then -- если подкреплений еще не было
      BlockGame()
      MoveCamera(21, 67, 0, 0, 0, 0, 1, 1, 1)
      sleep(5)
      MessageBox(wPath..'Dwells/reinfs_msg.txt', 'UnblockGame') -- даем инфу о них
      moriton.first_reinf = 1
    end
    if not IsObjectExists('reinfs') then -- если объекта подкреплений еще нет
      CreateMonster('reinfs', CREATURE_PIXIE, 10 * dwells.counts[1], 21, 67, 0, 0, 0, 0, not nil) -- создаем его
      sleep()
      AddObjectCreatures('reinfs', CREATURE_BLADE_JUGGLER, 8 * dwells.counts[2]) -- добавляем существ в зависимости от числа двеллингов
      ...
      if 0.5 * dwells.counts[4] >= 1 then -- казалось бы, причем тут quanity cannot be negative?
        AddObjectCreatures('reinfs', CREATURE_GREEN_DRAGON, 0.5 * dwells.counts[4])
      end
    else -- если уже существует
      AddObjectCreatures('reinfs', CREATURE_PIXIE, 10 * dwells.counts[1]) -- просто добавляем существ
...
-- отвечает за передвижение осаждающих некромантов
...
-- разрушение города после осады Айнуреля
function PostSiegeMoritonDestroy()
  local d_lvl = GetGameVar('destruction_lvl') + 0 -- получаем инфу из комбата
  print('Степень разрушений: ', d_lvl)
  if d_lvl == 0 then return end -- ничего не разрушено/битва была не в городе - выход
  ...
  if d_lvl == 1 then -- низкий уровень разрушений
    DestroyTownBuildingToLevel('Moriton', TOWN_BUILDING_FORT, 1) -- сносятся укрепления до минимума
  elseif d_lvl == 2 then -- большие разрушения
    DestroyTownBuildingToLevel('Moriton', TOWN_BUILDING_FORT, 0) -- минус все укрепления
    DestroyTownBuildingToLevel('Moriton', TOWN_BUILDING_TOWN_HALL, 2) -- золото до ратуши
  elseif d_lvl == 3 then -- тотальные разрушения
    DestroyTownBuildingToLevel('Moriton', TOWN_BUILDING_FORT, 0) -- минус укрепления
    DestroyTownBuildingToLevel('Moriton', TOWN_BUILDING_TOWN_HALL, 1) -- минус все до дома старейшин
    for i = TOWN_BUILDING_DWELLING_1, TOWN_BUILDING_DWELLING_3 + diff do -- сносятся также грейженые двеллинги
...
-- логика разговора с начальником стражи Моритона
SetObjectEnabled('war_lord', nil)
Touch.SetFunction('war_lord',
function(hero, object)
  if hero == 'Greta' then
    -- в теории можно убить Клемиру без взятого квеста, так что нужно обработать это отдельно
    if moriton.remove_def ~= 0 and not isActive(moriton.names.kklem) then
      -- если какой - нибудь из предыдущих квестов активен(опять же, в теории можно вообще ни одного квеста не брать)
      if isActive(moriton.names.u_at) then
        finishQuest(moriton.names.u_at, hero) -- просто завершить его
      end
      ...
      function(hero, object)
        ShowFlyingSign(rtext('Продолжаем сражаться, госпожа!'), object, -1, 7.0)
      end)
      return
    end
    ----------------------------------------------------------------------------
    if isUnknown(moriton.names.u_at) then -- первый раз зашли?
    ...
    elseif getProgress(moriton.names.u_at) == 8 then -- все стеки убили?
    ...
    elseif getProgress(moriton.names.u_path) == 2 then -- исследовали туннель?
    ...
    elseif getProgress(moriton.names.kklem) == 1 then -- убили Клемиру?
...
-- настройка стеков, которые нужно убить по квесту
...
-- проверка числа убитых стеков. Сделал так, чтобы отдельно не проверять, если
-- к моменту взятия квеста все уже убиты
...
-- в триггер нового дня
function CheckMoritonRemoveDef(day)
  if day == moriton.remove_def then
    moriton.remove_def = -1 -- флаг, что Карлам может теперь вернуться с артами
    for i = 1, 3 do
    ...
    MessageBox(rtext('Защитные чары Моритон пришли в норму. Теперь город снова доступен.'))
  end
end
...
function(hero, object)
  if isUnknown(moriton.names.u_path) then -- если квеста нет
    MessageBox(moriton.paths.u_path..'cant_enter.txt') -- то войти нельзя
  else
    SetObjectPosition(hero, 29, 52, 1, 0)
    if moriton.first_enter == 0 then -- проверяем на первый вход
...
-- дошли до выхода - видим, что он заблокирован
...
-- логика разговора с ведьмой
SetObjectEnabled('witch2', nil)
Touch.SetFunction('witch2',
function(hero, object)
  if hero == 'Greta' then
    if isUnknown(moriton.names.witch) then -- первый раз зашли?
    ...
    elseif getProgress(moriton.names.witch) == 1 then -- артефакты добыты?
      ...
      Touch.OverrideFunction(object, 1,
      function(hero, object)
        ShowFlyingSign(rtext('Да не оставит храбрость ваших воинов, госпожа!'), object, -1, 7.0)
      end)
    end
  end
end)
  ...
  if answer < 1 then
    ...
    if answer == 1 then
      if GetHeroSkillMastery('Greta', SKILL_SORCERY) < 3 then
        ...
        ShowFlyingSign(rtext('<color=red>Вы обучились чародейству!'), 'Greta', -1, 5.0)
        moriton.wsp_learned = 1
      else
        ShowFlyingSign(rtext('<color=red>Вы уже идеально владеете этим!'), 'Greta', -1, 5.0)
      end
    elseif answer == 2 then
      if GetHeroSkillMastery('Greta', SKILL_SUMMONING_MAGIC) < 3 then
        GiveHeroSkill('Greta', SKILL_SUMMONING_MAGIC)
        ShowFlyingSign(rtext('<color=green>Вы обучились магии Призыва!'), 'Greta', -1, 5.0)
        moriton.wsp_learned = 1
      else
        ShowFlyingSign(rtext('<color=green>Вы уже идеально владеете этим!'), 'Greta', -1, 5.0)
      end
    elseif answer == 3 then
      if GetHeroSkillMastery('Greta', SKILL_WAR_MACHINES) < 3 then
        GiveHeroSkill('Greta', SKILL_WAR_MACHINES)
        ShowFlyingSign(rtext('<color_neutral>Вы обучились управлению машинами!'), 'Greta', -1, 5.0)
        moriton.wsp_learned = 1
      else
        ShowFlyingSign(rtext('<color_neutral>Вы уже идеально владеете этим!'), 'Greta', -1, 5.0)
      end
    else
      ShowFlyingSign(rtext('<color_default>Как-нибудь в другой раз...'), 'Greta', -1, 5.0)
    end
  end
end

-- проверка наличия артефактов у Греты
