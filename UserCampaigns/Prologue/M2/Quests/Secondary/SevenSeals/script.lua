...
-- возможные существа-рохи в зависимости от сложности
CREATURE_CURSED_ROCK_EASY = 2400
CREATURE_CURSED_ROCK_MEDIUM = 2401
CREATURE_CURSED_ROCK_HARD = 2402
CREATURE_CURSED_ROCK_HEROIC_FWT = 2403 -- fire/water res    
CREATURE_CURSED_ROCK_HEROIC_FAR = 2404 -- fire/air res
CREATURE_CURSED_ROCK_HEROIC_FER = 2405 -- fire/earth res
CREATURE_CURSED_ROCK_HEROIC_WTAR = 2406 -- water/air res
CREATURE_CURSED_ROCK_HEROIC_WTER = 2407 -- water/earth res
CREATURE_CURSED_ROCK_HEROIC_ARER = 2408 -- air/earth res
...
seven_seals =
{
    ...
    function(hero)
        ...
        MiniDialog.Start(
            seven_seals.path.dialog.."StartDialog/", 
            PLAYER_1, 
            seven_seals.dialogs.StartDialog, 
            (Quest.IsActive(ghost_hope.name) and "ghost_quest_known" or nil)
        )
        ...
        Quest.SetObjectQuestmark("blood_temple_01", QUESTMARK_OBJ_NEW_PROGRESS, 10)
    end,
    ...
    function()
        ...
        while seven_seals.elem_trap_active == 1 do
            for i, emitter in {'wind_01', 'wind_02', 'wind_03'} do
                FX.Play('Lightning_bolt', emitter)
                sleep(4)
            end
            sleep()
        end
    ...
    function()
        ...
        while seven_seals.elem_trap_active == 1 do
            FX.Play('Firewall', 'fire_02', 'fire_02_fx')
            sleep(10)
            FX.Play('Firewall_end', 'fire_02')
    ...
    function(hero, region)
        ...
        if hero == "Karlam" then
            ...
            if Quest.IsUnknown(seven_seals.name) then
                if Quest.IsUnknown(ghost_hope.name) then
                    dialog_alt_set = nil
                else
                    dialog_alt_set = "seven_unknown_ghost_known"
                end
            else
                if Quest.IsUnknown(ghost_hope.name) then
                    dialog_alt_set = "seals_known_ghost_unknown"
                else
                    dialog_alt_set = "seven_known_ghost_known"
                end
            end
            ...
            sleep(30)
            FX.Play('Phantom_out', "Karlam")
            sleep(10)
            FX.Play('Phantom_in', "Karlam", '', 2, 2)
            sleep(8)
    ...
    function ()
        errorHook(
        function()
            print("<color=red>Error: <color=green>M1_Q_SevenSeals.GetRockVariant")
        end)
        if initialDifficulty == DIFFICULTY_EASY then
            return CREATURE_CURSED_ROCK_EASY
        elseif initialDifficulty == DIFFICULTY_NORMAL then 
            return CREATURE_CURSED_ROCK_MEDIUM
        elseif initialDifficulty == DIFFICULTY_HARD then
            return CREATURE_CURSED_ROCK_HARD 
        else
            local res = Random.FromSelection(
                CREATURE_CURSED_ROCK_HEROIC_FWT,
                CREATURE_CURSED_ROCK_HEROIC_FAR,
                CREATURE_CURSED_ROCK_HEROIC_FER,
                CREATURE_CURSED_ROCK_HEROIC_WTAR,
                CREATURE_CURSED_ROCK_HEROIC_WTER,
                CREATURE_CURSED_ROCK_HEROIC_ARER
            )
            return res
        end
    end,
    ...
    function(hero, region)
        ...
        if MCCS_StartCombat(hero, nil, 1, {rock_variant, 1}, 1, nil, nil, 1) then
            SetGameVar('cursed_rock_fight', '0')
            BlockGame()
            FX.Play('Phantom_out', hero)
            sleep(15)
            Object.Show(PLAYER_1, "Karlam")
            sleep()
            FX.Play('Phantom_in', "Karlam")
            sleep(10)
            ...
            sleep(5)
            for i = 1, 30 do
                FX.Play('Arcane_crys_death', 'sealed_tomb', '', 5.5 + (random(3) * Random.FromSelection(1, -1)), -7.5, random(15) + 1)
                sleep()
            end
            ...
            -- диалог
            MiniDialog.Start(
                seven_seals.path.dialog..'SealBrokenDialog/', 
                PLAYER_1, 
                seven_seals.dialogs.SealBrokenDialog, 
                (Quest.IsActive(seven_seals.name) and "tavern_quest_known" or nil)
            )
            sleep()
